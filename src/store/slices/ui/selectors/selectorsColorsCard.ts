...
import { getAllAssetFromCard, getSelectedDataByKeyPermission } from "./selectors";
...
export const getColorsFromCard = (keyItemPermission: string) => (state: RootState) => {
    ...
    const availableColorsData = colorsData.filter(color => availableColors.includes(color.name));
