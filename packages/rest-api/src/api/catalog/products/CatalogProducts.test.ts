...
import { CatalogProduct, CatalogProductListing } from './CatalogProducts.js';
...
describe('CatalogProducts', () => {
  ...
  describe('healthcheck()', () => {
    it('it should return a 200', async () => {
      const response = await client.catalog.healthcheck();
  ...
  describe('getById(id)', () => {
    it('it should throw an error if an invalid ID is provided', async () => {
      const result = () => client.catalog.products.getById('fail');
      expect(result).toThrow();
    });

    it('it should return a valid result when provided a valid assetId', async () => {
      const response = await client.catalog.products.getById(asset1.id);
      expect(response.status).toBe(200);
      expect(response.data).toMatchSchema(CatalogProduct);
    });

    it('it should return a 404 when given a wrong assetId', async () => {
      const response = await client.catalog.products.getById(WRONG_ID);
  ...
  describe('get()', () => {
    it('it should return a list of products', async () => {
      const response = await client.catalog.products.get();
      expect(response.status).toBe(200);
      expect(response.data).toMatchSchema(CatalogProductListing);
    });
  });
});
