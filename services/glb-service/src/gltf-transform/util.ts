...
export let commandExists = (cmd: string) =>
  _commandExists(cmd).catch(() => false);
...
export function mockCommandExists(
  _commandExists: (n: string) => Promise<boolean>
): void {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  commandExists = _commandExists as any;
}

export function mockWaitExit(
  _waitExit: (process: ChildProcess) => Promise<[unknown, string, string]>
): void {
  waitExit = _waitExit;
}

export async function _waitExit(
  ...
  const status = await new Promise((resolve, _neverUsed) => {
