...
function createParams(
  ...
  if (
    slots.find((slot) =>
      micromatch.isMatch(slot, '*normal*', MICROMATCH_OPTIONS)
    )
  ) {
  ...
  if (options.jobs && options.jobs > 1 && numTextures > 1) {
    ...
    const threads = Math.max(
      2,
      Math.min(NUM_CPUS, (3 * NUM_CPUS) / numTextures)
    );
...
async function checkKTXSoftware(logger: ILogger): Promise<string> {
  ...
  if (status !== 0 || !semver.valid(semver.clean(version))) {
    throw new Error(
      'Unable to find "toktx" version. Confirm KTX-Software is installed.'
    );
  } else if (semver.lt(semver.clean(version)!, KTX_SOFTWARE_VERSION_MIN)) {
    logger.warn(
      `toktx: Expected KTX-Software >= v${KTX_SOFTWARE_VERSION_MIN}, found ${version}.`
    );
  } else {
